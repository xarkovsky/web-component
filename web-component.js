 ...
 *
 * This listing contains code from the book
 * "Core HTML5 Canvas" - David Geary - 2012.
 * 
 */
...
var form   		 = O('form-1'        );
var ranger 		 = O('ranger'        );
var output 		 = O('ranger-output' );

var stripePink1  = O('front-pink-N1' );
var stripePink2  = O('front-pink-N2' );
var stripeLime3  = O('front-lime-N3' );
var stripePink4  = O('back-pink-N4'  );
var stripeLime5  = O('back-lime-N5'  );
var stripeLime6  = O('middle-lime-N6');
var stripePink7  = O('back-pink-N7'  );
var stripePink8  = O('back-pink-N8'  );
var stripePink9  = O('back-pink-N9'  );
var stripeLIme10 = O('back-lime-N10' );
var stripePink11 = O('back-pink-N11' );


// C. CREATE GROUP function.
function createGroup ()
{
	...
	for ( i = 0; i < len; i++ )
	{
		arrayOfGroup.push(arguments[i]);
		//log ( 'arguments[ ' + i + '] : ' + arguments[i] );
		arrayOfGroup[i].testName = arrayOfGroup[i].innerHTML;
	}
...
// C.1 CREATE GROUPS.
var groupFront  = createGroup( stripePink1, stripePink2, stripeLime3 );
var groupBack   = createGroup( stripePink8, stripePink9,
						   	   stripePink4, stripeLime5, 
						   	   stripePink7, stripeLIme10, 
						   	   				stripePink11 );

// D. STYLES SETUP function.
function setupStylesForEveryStripe ( group, startBlurForStripe, width, height)
{
	var valueOfBlur;

	for ( var i = 0; i < group.length; i++)
	{
		group[i].startBlurForStripe = startBlurForStripe;
		valueOfBlur = 'blur(' + startBlurForStripe + 'px )' ;
		group[i].style.filter = valueOfBlur;
		group[i].style.width  = width + 'px';
...
// D.1 SET STYLES.
setupStylesForEveryStripe( groupFront, 1            );
setupStylesForEveryStripe( groupBack , 30, 20, 150  );

// E. SET NEW VALUE  function.
function updateNewValueBlurForStripe ( group, direction, boundary )
{
	var startBlurForStripe;
	var endBlurForStripe;
	var valueOfBlur;
	var clockWise;
	var k ;

	if ( direction == 'revers'  )  { k = -1 }
	if ( direction == 'forward' )  { k =  1 }
	
	for ( var i = 0; i < group.length; i++)
	{
		
		startBlurForStripe = group[i].startBlurForStripe;
		rangerValue        = parseInt(slider.knobPercent * maxRangeValue);
		endBlurForStripe   = startBlurForStripe + k * rangerValue;
		
		if ( direction == 'revers' &&  rangerValue > boundary )
		{
			endBlurForStripe = 0;
		}

		valueOfBlur              = 'blur(' + endBlurForStripe + 'px )' ;
		group[i].style.filter    = valueOfBlur;
		group[i].style.transform = 'translateZ(' + rangerValue * 2 +
								   'px) rotateZ(50deg) skewX(18deg)';
...
{
	updateNewValueBlurForStripe( groupFront, 'forward'     );
	updateNewValueBlurForStripe( groupBack , 'revers' , 35 );
}
...
COMPONENT.Slider.prototype = 
{
   ...
   // J. ARRAY LISTENERS AND FIRE EVENTS
